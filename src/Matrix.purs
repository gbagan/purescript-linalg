...
fromArray m = case Array.uncons m of
    Just {head, tail} | not (Array.null head) && Array.all (\line -> Array.length line == Array.length head) tail ->
        Matrix (Array.length m) (Array.length head) m
    _ -> Invalid
...
column j m@(Matrix _ c _) = fromMaybe V.invalid $ V.fromArray <$> traverse (\i -> elem' i j m) (0..(c-1))
