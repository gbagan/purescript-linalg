...
newtype Matrix a = Matrix { r :: Int, c :: Int, m :: Array (Array a) }
...
fromArray m = case uncons m of
    Just {head, tail} | not (null head) && all (\line -> length line == length head) tail ->
        Matrix {r: length m, c: length head, m }
    _ -> invalid
...
row i (Matrix {m}) = fromMaybe V.invalid $ V.fromArray <$> m !! i

column :: forall a. Int -> Matrix a -> V.Vector a
column j m@(Matrix {r}) = fromMaybe V.invalid $ V.fromArray <$> traverse (\i -> elem' i j m) (0..(r-1))
...
swapRows r1 r2 m = mapWithIndex fn m where
    fn i j x | i == r1 = runFn3 unsafeElem m r2 j
...
gaussJordan m@(Matrix {r, c}) = {mat: res.mat, det: res.det} where
...
augmentedMatrix m@(Matrix {r, c}) = fromFunction r (r + c) fAug where
...
imker m@(Matrix {r, c}) = {im, ker} where
...
solveLinearSystem' m@(Matrix {r, c}) b = 
    ...
    if r == 0 || (0..(c-1) # all \j -> runFn3 unsafeElem echelon (r'-1) j == zero) then
        ...
        let toBeUpdated = 0..(r'-1) <#> (\i ->
            ...
            let k = fromMaybe 0 $ 0..(c-1) # find \j -> runFn3 unsafeElem echelon i j == one in
